...
image:https://codecov.io/gh/mihxil/i18n-iso-639/branch/main/graph/badge.svg[codecov,link=https://codecov.io/gh/mihxil/i18n-iso-639-3]
...
Optional<ISO_639_Code> languageCode = ISO_639_Code.getByPart3("nld");

ISO_639_Code languageCode = ISO_639_Code.languageCode("nl");
...
| Refactoring
| 2024-3
|  Added enum for ISO-639-1 codes and ISO-639-4. Dropped the -3 from the artifact id. 
|   Made syntax forward compabitble with records. So, getters like `getPart1()`) are dropped in favor of `part1()`. `LanguageCode` itself is now an interface. This may be backported to 1.2 for javax compatibility. 
